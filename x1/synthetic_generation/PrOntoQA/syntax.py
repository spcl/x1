...
class AmbiguousParseError(Exception):
    def __init__(self, tokens, logical_forms):
        ...
        super().__init__(
            "Sentence {} has multiple semantic parses {}".format(
                tokens, [fol.fol_to_tptp(lf) for lf in logical_forms]
            )
        )
...
def formula_to_np_prime(
    formula, morphology, quantified_variable, is_plural, no_adjectives
):
    if (
        type(formula) == fol.FOLFuncApplication
        and len(formula.args) == 1
        and type(formula.args[0]) == fol.FOLVariable
        and formula.args[0].variable == quantified_variable
    ):
        ...
        type(formula) == fol.FOLAnd
        and type(formula.operands[0]) == fol.FOLFuncApplication
        and not morphology.is_noun(formula.operands[0].function)
        and not no_adjectives
    ):
        ...
        return SyntaxNode(
            ...
            [
                ...
                formula_to_np_prime(
                    other, morphology, quantified_variable, is_plural, no_adjectives
                ),
            ],
        )
    else:
        ...
        pdb.set_trace()
        raise Exception(
            "formula_to_np_prime ERROR: Unsupported formula type ({}).".format(
                type(formula)
            )
        )
...
def can_be_adjp_np(formula, morphology):
    if type(formula) == fol.FOLFuncApplication:
        ...
        return all(
            [
                type(operand) == fol.FOLFuncApplication
                and not morphology.is_noun(operand.function)
...
def formula_to_np(
    ...
    quantified_variable,
    is_plural,
    is_universally_quantified,
    is_negated,
    no_adjectives,
):
    if (type(formula) == fol.FOLOr and is_universally_quantified and not is_plural) or (
        (no_adjectives or not can_be_adjp_np(formula, morphology))
        and type(formula) == fol.FOLAnd
        and is_universally_quantified
        and not is_plural
    ):
        child_nodes = []
        for operand in formula.operands:
            if type(operand) == fol.FOLFuncApplication and not morphology.is_noun(
                operand.function
            ):
                # this operand is predicative (adjectival phrase)
                child_nodes.append(
                    formula_to_adjp(operand, morphology, quantified_variable)
                )
            else:
        ...
        has_commas = random.choice([True, False])
        ...
        if len(formula.operands) > 2:
            if has_commas:
                ...
                child_nodes = result
                child_nodes.insert(
                    len(child_nodes) - 1, SyntaxNode("CC", [SyntaxNode(cnj)])
                )
            else:
                result = [SyntaxNode("CC", [SyntaxNode(cnj)])] * (
                    len(child_nodes) * 2 - 1
                )
                result[0::2] = child_nodes
                child_nodes = result
        ...
        return SyntaxNode(
            ...
            [
                SyntaxNode(
                    "NP",
                    [SyntaxNode("NP'", [SyntaxNode("N", [SyntaxNode("everything")])])],
                ),
                SyntaxNode(
                    "SBAR",
                    [
    ...
    np_prime = formula_to_np_prime(
        formula, morphology, quantified_variable, is_plural, no_adjectives
    )
    if is_negated:
        return SyntaxNode("NP", [SyntaxNode("DET", [SyntaxNode("no")]), np_prime])
        ...
        if is_plural:
            if random.randrange(1) == 0:
                ...
                return SyntaxNode(
                    "NP", [SyntaxNode("DET", [SyntaxNode("all")]), np_prime]
                )
        else:
            quantifier = random.choice(["each", "every"])
            return SyntaxNode(
                "NP", [SyntaxNode("DET", [SyntaxNode(quantifier)]), np_prime]
            )
    else:
        if is_plural:
            return SyntaxNode("NP", [np_prime])
        else:
            return SyntaxNode("NP", [SyntaxNode("DET", [SyntaxNode("a")]), np_prime])
...
def formula_to_adjp(formula, morphology, quantified_variable):
    if (
        ...
        raise Exception(
            "formula_to_adjp ERROR: Unsupported formula type ({}).".format(
                fol.fol_to_tptp(formula)
            )
        )


def formula_to_vp_arg(
    formula, morphology, quantified_variable, is_plural, no_adjectives
):
    if type(formula) == fol.FOLFuncApplication and not morphology.is_noun(
        formula.function
    ):
        return formula_to_adjp(formula, morphology, quantified_variable)
    elif type(formula) == fol.FOLOr or (
        type(formula) == fol.FOLAnd
        and (no_adjectives or not can_be_adjp_np(formula, morphology))
    ):
        child_nodes = []
        for operand in formula.operands:
            if type(operand) == fol.FOLFuncApplication and not morphology.is_noun(
...
def formula_to_clause(formula, morphology, no_adjectives=False, invert=False):
    if type(formula) == fol.FOLForAll and type(formula.operand) == fol.FOLIfThen:
        ...
        if invert:
            is_plural = False
        elif type(formula.operand.antecedent) == fol.FOLOr or (
            type(formula.operand.antecedent) == fol.FOLAnd
            and (
                no_adjectives
                or not can_be_adjp_np(formula.operand.antecedent, morphology)
            )
        ):
            is_plural = False
        ...
        if is_negated:
            right = formula_to_vp_arg(
                formula.operand.consequent.operand,
                morphology,
                formula.variable,
                is_plural,
                no_adjectives,
        ...
        if is_negated:
                ...
                return SyntaxNode(
                    ...
                    [
                        ...
                        SyntaxNode(
                            ...
                            [
                                SyntaxNode(
                                    "ADVP", [SyntaxNode("ADV", [SyntaxNode("not")])]
                                ),
                                right,
                            ],
                        ),
            ...
            if invert:
                ...
                return SyntaxNode(
                    ...
                    [
                        left,
                        SyntaxNode("VP", [SyntaxNode("V", [SyntaxNode(verb)]), right]),
                    ],
                )

    elif (
        type(formula) == fol.FOLNot
        and type(formula.operand) == fol.FOLExists
        and type(formula.operand.operand) == fol.FOLAnd
    ):
        # determine if the clause will be singular or plural
        is_plural = random.choice([True, False])
        ...
        # generate the subject and object phrases
        left = formula_to_np(
            formula.operand.operand.operands[0],
            morphology,
            formula.operand.variable,
            is_plural,
            False,
            True,
            no_adjectives,
        ...
        if invert:
            return SyntaxNode(
                "S",
                [SyntaxNode("V", [SyntaxNode(verb)]), left, SyntaxNode("VP", [right])],
            )
        else:
            return SyntaxNode(
                "S",
                [left, SyntaxNode("VP", [SyntaxNode("V", [SyntaxNode(verb)]), right])],
            )

    elif (
        type(formula) == fol.FOLFuncApplication
        and formula.function == "ASSUME"
        and len(formula.args) == 1
    ):
        subclause = formula_to_clause(
            formula.args[0], morphology, no_adjectives, invert
        )
        return SyntaxNode(
            "S",
            [SyntaxNode("VP", [SyntaxNode("V", [SyntaxNode("assume")]), subclause])],
        )
        ...
        return SyntaxNode(
            ...
            [
                SyntaxNode(
                    "NP", [SyntaxNode("NP'", [SyntaxNode("NN", [SyntaxNode("this")])])]
                ),
                SyntaxNode(
                    "VP",
                    [
                        ...
                        SyntaxNode(
                            "PP", [SyntaxNode("P", [SyntaxNode("with")]), subclause]
                        ),
                    ],
                ),
            ],
        ...
        type(formula) == fol.FOLFuncApplication
        and formula.function == "BACKTRACK"
        and len(formula.args) == 1
    ):
        subclause = formula_to_clause(
            formula.args[0], morphology, no_adjectives, invert
            ...
            [
                SyntaxNode(
                    ...
                    [
                        ...
                        SyntaxNode(
                            "PP", [SyntaxNode("P", [SyntaxNode("to")]), subclause]
                        ),
                    ],
                )
            ],
        ...
        type(formula) == fol.FOLFuncApplication
        and formula.function == "START_OVER"
        and len(formula.args) == 0
    ):
        return SyntaxNode(
            "S",
            [
                SyntaxNode(
                    ...
                    [
                        ...
                        SyntaxNode("ADVP", [SyntaxNode("ADV", [SyntaxNode("again")])]),
                    ],
                )
            ],
        )
        ...
        return SyntaxNode(
            "S",
            [SyntaxNode("CC", [SyntaxNode("since")]), first, SyntaxNode(","), second],
        )

    elif (
        type(formula) == fol.FOLFuncApplication
        and len(formula.args) == 1
        and type(formula.args[0]) == fol.FOLConstant
    ):
        right = formula_to_vp_arg(
            fol.FOLFuncApplication(formula.function, [fol.FOLVariable(1)]),
            morphology,
            1,
            False,
            no_adjectives,
        ...
        if invert:
                    ...
                    SyntaxNode(
                        ...
                        [
                            SyntaxNode(
                                ...
                                [
                                    SyntaxNode(
                                        "NN", [SyntaxNode(formula.args[0].constant)]
                                    )
                                ],
                            )
                        ],
            ...
            return SyntaxNode(
                ...
                [
                    ...
                    SyntaxNode("VP", [SyntaxNode("V", [SyntaxNode("is")]), right]),
                ],
            )

    elif (
        type(formula) == fol.FOLNot
        and type(formula.operand) == fol.FOLFuncApplication
        and len(formula.operand.args) == 1
    ):
        right = formula_to_vp_arg(
            fol.FOLFuncApplication(formula.operand.function, [fol.FOLVariable(1)]),
            morphology,
            1,
            False,
            no_adjectives,
                        ...
                        [
                            SyntaxNode(
                                ...
                                [
                                    SyntaxNode(
                                        "NN",
                                        [SyntaxNode(formula.operand.args[0].constant)],
                                    )
                                ],
                            )
                        ],
        ...
        for operand in formula.operands:
            if (
                ...
                and type(operand.args[0]) == fol.FOLConstant
                and (entity == None or entity == operand.args[0])
            ):
                entity = operand.args[0]
                right_lf.append(
                    fol.FOLFuncApplication(operand.function, [fol.FOLVariable(1)])
                )
            else:
        ...
        if expressible_as_np:
            ...
            right = formula_to_vp_arg(
                right_formula, morphology, 1, False, no_adjectives
            )
            if invert:
                return SyntaxNode(
                    "S",
                    [
                        ...
                        SyntaxNode(
                            ...
                            [
                                SyntaxNode(
                                    "NP'",
                                    [SyntaxNode("NN", [SyntaxNode(entity.constant)])],
                                )
                            ],
                        ),
                        SyntaxNode("VP", [right]),
        ...
        child_nodes = [
            formula_to_clause(operand, morphology, no_adjectives, invert=False)
            for operand in formula.operands
        ]
        has_commas = random.choice([True, False])
        ...
        if len(formula.operands) > 2:
            if has_commas:
                ...
                child_nodes = result
                child_nodes.insert(
                    len(child_nodes) - 1, SyntaxNode("CC", [SyntaxNode(coordinator)])
                )
            else:
                result = [SyntaxNode("CC", [SyntaxNode(coordinator)])] * (
                    len(child_nodes) * 2 - 1
                )
                result[0::2] = child_nodes
                child_nodes = result
...
def inflect(tokens, end_punctuation):
    for i in range(len(tokens)):
        if tokens[i] == "a":
            if (
                i + 1 < len(tokens)
                and tokens[i + 1][0] in {"a", "e", "i", "o", "u"}
                and not tokens[i + 1].startswith("eu")
            ):
                tokens[i] = "an"
    sentence = tokens[0]
...
def parse_np_prime(tokens, index, morphology):
    ...
    if compound != None and morphology.is_noun(compound):
        ...
        is_plural = None if morphology.is_noun(compound) else True
        lf = fol.FOLFuncApplication(
            morphology.to_root(compound.lower()), [fol.FOLVariable(1)]
        )
        index += 2
    elif (
        compound != None
        and morphology.is_noun(tokens[index].lower())
        and morphology.is_noun(tokens[index + 1])
    ):
        ...
        compound != None
        and morphology.is_noun(tokens[index].lower())
        and morphology.is_plural_noun(tokens[index + 1])
    ):
        ...
        lf = fol.FOLAnd(
            [
                ...
                fol.FOLFuncApplication(
                    morphology.to_root(tokens[index + 1]), [fol.FOLVariable(1)]
                ),
            ]
        )
        index += 2
        ...
        lf = fol.FOLFuncApplication(
            morphology.to_root(tokens[index].lower()), [fol.FOLVariable(1)]
        )
        index += 1
    else:
...
def parse_np(tokens, index, morphology):
    if tokens[index].lower() in {"each", "every"}:
        ...
        tokens[index].lower() == "everything"
        and tokens[index + 1] == "that"
        and tokens[index + 2] == "is"
    ):
        ...
        while index < len(tokens):
            ...
            (
                ...
                operand_is_plural,
                operand_is_negated,
                operand_is_quantified,
            ) = parse_np(tokens, index, morphology)
            if operand == None:
    ...
    if det_is_plural != None and is_plural != None and is_plural != det_is_plural:
        return (
            ...
            None,
            None,
        )  # the grammatical number of the determiner and the noun don't agree
...
def parse_vp_arg(tokens, index, morphology):
    ...
    while index < len(tokens):
        ...
        if tokens[index] == "not":
            ...
            negated = False
        (operand, new_index, operand_is_plural, is_negated, is_quantified) = parse_np(
            tokens, index, morphology
        )
        if type(operand) == fol.FOLConstant:
            ...
            if (
                operand == None
                or (type(operand) == fol.FOLAnd and is_disjunction)
                or (type(operand) == fol.FOLOr and is_conjunction)
            ):
    ...
    if len(operands) == 1 and not is_conjunction and not is_disjunction:
        ...
        if operand_indices[0] == -1:
            ...
            return [
                (operands[0], operand_indices[0], operand_plural[0]),
                (fol.FOLAnd(operands), index, is_plural),
            ]
    elif is_disjunction:
        if operand_indices[0] == -1:
            ...
            return [
                (operands[0], operand_indices[0], operand_plural[0]),
                (fol.FOLOr(operands), index, is_plural),
            ]
    else:
        return []
...
def parse_clause(
    tokens, index, morphology, can_be_coordinated=True, complete_sentence=False
):
    if tokens[index].lower() == "is":
        ...
        outputs = parse_clause(
            tokens, index + 1, morphology, complete_sentence=complete_sentence
        )
        return [
            (fol.FOLFuncApplication("ASSUME", [lf]), index, invert)
        ...
        if len(outputs) == 1 and outputs[0][1] == len(tokens):
            ...
            outputs = parse_clause(
                tokens, index + 1, morphology, can_be_coordinated=False
            )
        ...
        for lf, new_index, invert in outputs:
            ...
            if tokens[new_index] == ",":
                new_index += 1
            if new_index + 3 < len(tokens) and tokens[new_index : (new_index + 4)] == [
                "the",
                "statement",
                "is",
                "proven",
                ...
                right_outputs = parse_clause(
                    tokens, new_index, morphology, complete_sentence=complete_sentence
                )
                for right_lf, right_index, right_invert in right_outputs:
                    new_outputs.append(
                        (
                            fol.FOLFuncApplication("SINCE", [lf, right_lf]),
                            right_index,
                            right_invert,
                        )
                    )
        ...
        if index == len(tokens):
            ...
            index += 1
        outputs = parse_clause(
            tokens, index, morphology, complete_sentence=complete_sentence
        )
        return [
            (fol.FOLFuncApplication("HOWEVER", [lf]), index, invert)
            for (lf, index, invert) in outputs
        ]
        ...
        return [
            (fol.FOLFuncApplication("THEREFORE", [lf]), index, invert)
            for (lf, index, invert) in outputs
        ]
    elif (
        index + 1 < len(tokens)
        and tokens[index].lower() == "this"
        and tokens[index + 1] == "proves"
    ):
        outputs = parse_clause(
            tokens, index + 2, morphology, complete_sentence=complete_sentence
        )
        return [
            (fol.FOLFuncApplication("THEREFORE", [lf]), index, invert)
            for (lf, index, invert) in outputs
        ...
        index + 2 < len(tokens)
        and tokens[index].lower() == "this"
        and tokens[index + 1] == "contradicts"
        and tokens[index + 2] == "with"
    ):
        outputs = parse_clause(
            tokens, index + 3, morphology, complete_sentence=complete_sentence
        )
        return [
            (fol.FOLFuncApplication("CONTRADICTS", [lf]), index, invert)
            for (lf, index, invert) in outputs
        ]
    elif (
        index + 4 < len(tokens)
        and tokens[index].lower() == "this"
        and tokens[index + 1] == "contradicts"
        and tokens[index + 2] == "the"
        and tokens[index + 3] == "fact"
        and tokens[index + 4] == "that"
    ):
        outputs = parse_clause(
            tokens, index + 5, morphology, complete_sentence=complete_sentence
        )
        return [
            (fol.FOLFuncApplication("CONTRADICTS", [lf]), index, invert)
            for (lf, index, invert) in outputs
        ...
        index + 3 < len(tokens)
        and tokens[index].lower() == "this"
        and tokens[index + 1] == "is"
        and tokens[index + 2] == "a"
        and tokens[index + 3] == "contradiction"
    ):
        index += 4
        ...
        index + 1 < len(tokens)
        and tokens[index].lower() == "backtrack"
        ...
        return [
            (fol.FOLFuncApplication("BACKTRACK", [lf]), index, invert)
            for (lf, index, invert) in outputs
        ]
    elif (
        index + 1 < len(tokens)
        and tokens[index].lower() == "try"
        and tokens[index + 1] == "again"
    ):
        return [(fol.FOLFuncApplication("START_OVER", []), index + 2, False)]
    elif [token.lower() for token in tokens[index : (index + 6)]] == [
        "according",
        "to",
        "the",
        "given",
        ...
        return parse_clause(
            tokens, index + 6, morphology, complete_sentence=complete_sentence
        )
    else:
        is_plural = None
        invert = False

    (left_lf, index, np_is_plural, np_is_negated, is_quantified) = parse_np(
        tokens, index, morphology
    )
    if left_lf == None:
        return []
    ...
    for right_lf, index, vp_is_plural in vp_args:
        ...
        if type(left_lf) == fol.FOLConstant:
            ...
            if np_is_negated:
                ...
                lf = fol.FOLNot(
                    fol.FOLExists(1, fol.FOLAnd([left_lf] + remaining_conjuncts))
                )
            else:
                for _ in range(num_negations):
        ...
        if not can_be_coordinated:
            ...
            while len(queue) != 0:
                (operands, index, has_commas, is_conjunction, is_disjunction) = (
                    queue.pop()
                )
                if index == len(tokens):
                    coordination_outputs.append(
                        (operands, index, is_conjunction, is_disjunction)
                    )
                    continue
                old_index = index
                if tokens[index] == ",":
                ...
                if tokens[index] == "and":
                    ...
                    index = old_index
                    break
                coordinate_parses = parse_clause(
                    tokens, index, morphology, can_be_coordinated=False
                )
                for coordinate_lf, index, _ in coordinate_parses:
                    queue.append(
                        (
                            operands + [coordinate_lf],
                            index,
                            has_commas,
                            is_conjunction,
                            is_disjunction,
                ...
                if len(coordinate_parses) == 0:
                    coordination_outputs.append(
                        (operands, old_index, is_conjunction, is_disjunction)
                    )
            ...
            for operands, index, is_conjunction, is_disjunction in coordination_outputs:
                if len(operands) == 1:
                    ...
                    not is_disjunction
                    and len(operands) == 2
                    and type(operands[1]) == fol.FOLFuncApplication
                    and operands[1].function == "THEREFORE"
                ):
                    new_lf = fol.FOLFuncApplication(
                        "SINCE", [operands[0], operands[1].args[0]]
                    )
                elif is_conjunction:
                ...
                if index + 2 < len(tokens):
                    ...
                    if tokens[index] in {"so", "therefore"}:
                        (conclusion_lf, index, _) = parse_singleton_clause(
                            tokens,
                            index + 1,
                            morphology,
                        ...
                        if conclusion_lf == None:
                            ...
                            new_lf = fol.FOLFuncApplication(
                                "SINCE", [new_lf, conclusion_lf]
                            )
                    elif tokens[index] in ["because", "since", "as"]:
                        (premise_lf, index, _) = parse_singleton_clause(
                            tokens,
                            index + 1,
                            morphology,
                            complete_sentence=complete_sentence,
        ...
        outputs.extend(
            [
                (lf, index, invert)
                for lf, index in arg_outputs
                if not (complete_sentence and index != len(tokens))
            ]
        )
...
def parse_singleton_clause(
    ...
    outputs = parse_clause(
        ...
        index,
        morphology,
        can_be_coordinated=can_be_coordinated,
        complete_sentence=complete_sentence,
    )
    if len(outputs) == 0:
...
def parse_sentence(sentence, morphology, expect_invert=None):
    ...
    new_outputs = []
    for lf, index, invert in outputs:
        if (expect_invert != None and invert != expect_invert) or index != len(
            sentence
        ):
