...
class FOLIfThen(FOLFormula):
    ...
    def __ne__(self, other):
        if type(other) != FOLIfThen:
            return True
        return (
            self.antecedent != other.antecedent or self.consequent != other.consequent
        )
...
def substitute(formula, src, dst):
    def apply_substitute(f):
        if f == src:
            ...
            type(src) == FOLVariable
            and type(f) in [FOLForAll, FOLExists]
            and src.variable == f.variable
        ):
            return f
        else:
...
def free_variables(formula):
    ...
    def free_variables_visit(f):
        ...
        if (
            type(f) == FOLVariable
            and f.variable not in bound_variables
            and f.variable not in free_variables
        ):
...
def bound_variables(formula):
    ...
    def bound_variables_visit(f):
        ...
        if (
            type(f) == FOLForAll or type(f) == FOLExists
        ) and f.variable not in bound_variables:
            bound_variables.append(f.variable)
...
def parse_fol_term_from_tptp(string, pos, var_map):
    ...
    if string[pos:end] in var_map:
        ...
        if must_be_var:
            raise Exception(
                f"parse_fol_term_from_tptp ERROR at {pos+1}: Variable undeclared."
            )
    ...
    if string[end] == "(":
        ...
        while True:
            ...
            if string[end] == ",":
                ...
                raise Exception(
                    f"parse_fol_term_from_tptp ERROR at {end+1}: Expected a closing parenthesis or comma in argument array."
                )
...
def parse_fol_quantifier_from_tptp(string, pos, var_map):
    ...
    for var_name in var_names:
        if var_name in var_map:
            raise Exception(
                f"parse_fol_from_tptp ERROR at {pos+1}: Variable '{var_name}' already declared in this scope."
            )
...
def parse_fol_literal_from_tptp(string, pos, var_map):
    if string.startswith("?[", pos):
        (variables, formula, pos) = parse_fol_quantifier_from_tptp(
            string, pos + len("?["), var_map
        )
        ...
        if string[pos] != ")":
            raise Exception(
                f"parse_fol_from_tptp ERROR at {pos+1}: Expected a closing parenthesis."
            )
        pos += 1
    else:
...
def fol_to_tptp(formula):
    if type(formula) == FOLVariable:
        ...
        return (
            formula.function
            + "("
            + ",".join([fol_to_tptp(arg) for arg in formula.args])
            + ")"
        )
    elif type(formula) == FOLAnd:
        return (
            "("
            + " & ".join([fol_to_tptp(operand) for operand in formula.operands])
            + ")"
        )
        ...
        return (
            "("
            + fol_to_tptp(formula.antecedent)
            + " => "
            + fol_to_tptp(formula.consequent)
            + ")"
        )
        ...
        raise Exception(
            f"fol_to_tptp ERROR: Unrecognized formula type {type(formula)}."
        )
...
def unify(first, second, first_var_map, second_var_map):
    if type(first) != type(second):
        ...
        for i in range(len(first.operands)):
            if not unify(
                first.operands[i], second.operands[i], first_var_map, second_var_map
            ):
                return False
        ...
        return unify(
            first.antecedent, second.antecedent, first_var_map, second_var_map
        ) and unify(first.consequent, second.consequent, first_var_map, second_var_map)
    elif type(first) == FOLEquals:
        return unify_term(
            first.left, second.left, first_var_map, second_var_map
        ) and unify_term(first.right, second.right, first_var_map, second_var_map)
        ...
        for i in range(len(first.args)):
            if not unify_term(
                first.args[i], second.args[i], first_var_map, second_var_map
            ):
                return False
    else:
